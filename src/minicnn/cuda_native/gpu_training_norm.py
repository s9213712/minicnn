...
from minicnn.cuda_native.backward import _layernorm_backward_arrays
...
from minicnn.cuda_native.kernels import _layernorm_forward_array
...
def native_gpu_layernorm_linear_training_step(
    ...
    *,
    ...
    """Run Flatten + LayerNorm(reference shim) + optional activation + Linear(native) + SoftmaxCE + SGD."""
    ...
    norm_output, _cache = _layernorm_forward_array(
        flat_x,
        gamma=norm_weight_f32,
        beta=norm_bias_f32,
        normalized_shape=normalized_shape_t,
        eps=norm_eps,
    )
    ...
    norm_t = stage(norm_output, 'norm_output')
    ...
    try:
        ...
        grad_input_flat, grad_norm_weight, grad_norm_bias = _layernorm_backward_arrays(
            flat_x,
            grad_norm_host,
            gamma=norm_weight_f32,
            normalized_shape=normalized_shape_t,
        ...
        result_norm_output = norm_output if activation_key is None else runtime.stage_to_host(activated_t)
