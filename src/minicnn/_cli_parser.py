...
from minicnn._cli_output import _add_format_arg
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    p_eval_ckpt = sub.add_parser('evaluate-checkpoint', help='Evaluate a torch/flex checkpoint on a test set')
    p_eval_ckpt.add_argument('--config', type=str, default='configs/dual_backend_cnn.yaml')
    p_eval_ckpt.add_argument('--checkpoint', type=str)
    p_eval_ckpt.add_argument('--summary', type=str, help='Optional summary.json used to resolve best_model_path')
    p_eval_ckpt.add_argument('--device', choices=['auto', 'cpu', 'cuda'], default='auto')
    p_eval_ckpt.add_argument('--batch-size', type=int)
    p_eval_ckpt.add_argument('--test-data', type=str, help='Optional .npz test set with x/y arrays')
    p_eval_ckpt.add_argument('--test-data-normalized', action='store_true', help='Treat --test-data x as already normalized model input')
    p_eval_ckpt.add_argument('overrides', nargs='*', help='Overrides like dataset.data_root=/path/to/data')
    _add_format_arg(p_eval_ckpt)
