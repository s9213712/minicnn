...
- `inference/`: checkpoint inference helpers, including `predict_image.py` for real-image preprocessing + single-image prediction.
...
their schema before trying to reuse them. For torch/flex checkpoints you can
also run `minicnn evaluate-checkpoint --config <yaml> --checkpoint <file.pt>`
to measure test accuracy on the official dataset split, or
`python -u examples/inference/predict_image.py --config <yaml> --checkpoint <file.pt> --image <file>`
to preprocess a real image and inspect top-k predictions.
