...
Current training summaries also expose a minimal shared contract:
...
Consumers such as `minicnn evaluate-checkpoint` now validate that a supplied
summary file is really a `training_run_summary`, instead of treating any JSON
file with `best_model_path` as compatible.
...
Structured JSON output from `evaluate-checkpoint` now uses:
...
目前所有 training summary 也都對齊到一組最小共同欄位：
...
像 `minicnn evaluate-checkpoint` 這類 consumer，現在也會先驗證輸入檔確實是
`training_run_summary`，不再只因為某個 JSON 檔剛好有 `best_model_path` 就接受。
...
`evaluate-checkpoint` 的 JSON 輸出目前固定使用：
