...
- **`train.init_seed`**：torch/flex 在 `build_model()` 前會先設定 PyTorch seed，同一份 config 重跑會從相同初始權重開始。CUDA legacy 與 CPU/NumPy autograd 已各自使用 init seed。
- **Config overrides**：dotted CLI override 支援 `model.layers.1.out_features=7` 這類 list index；CUDA legacy 的非法數值欄位會回報 validation error，不再直接 traceback。
- **Boolean parsing**：`"false"`、`"0"` 等字串會正確解析為 false，涵蓋 dataset download、augmentation、AMP、optimizer helper flag 與 CUDA conv `pool` 欄位。
- **CUDA library switching**：同一 process 連續跑不同 `cuda_legacy` native variant 時會重設 native handle cache。
- **CUDA cleanup**：legacy CUDA 訓練即使在 final evaluation 前丟例外，也會透過外層 cleanup 釋放 device weights 與 velocity buffers。
- **`maxpool_backward_nchw`**：native code 新增 status-returning wrapper，舊的 void ABI 仍保留相容性。
