...
MiniCNN 也有自己的小型 CPU/NumPy autograd core，位置在 `src/minicnn/nn/tensor.py`。它支援 `Tensor.backward()` 的 reverse-mode autodiff，涵蓋 scalar/tensor arithmetic、broadcasting、matrix multiply、reductions、reshape、ReLU、`log_softmax`、`cross_entropy`、可訓練的 `Parameter`，以及不依賴 torch 的輕量 `SGD` optimizer。
...
| `src/minicnn/optim/` | 輕量 optimizer 介面；`SGD` 可更新 MiniCNN `Parameter`，不需要 torch。 |
